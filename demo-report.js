...
const counts = {
  critical: detectedVulnerabilities.filter((v) => v.severity === 'critical').length,
  high: detectedVulnerabilities.filter((v) => v.severity === 'high').length,
  medium: detectedVulnerabilities.filter((v) => v.severity === 'medium').length,
  low: detectedVulnerabilities.filter((v) => v.severity === 'low').length,
};
...
**Contract Address:** `0x742d35Cc6634C0532925a3b844Bc93948
**Network:** ETHEREUM
...
| LOW | ' + counts.low + ' |
| **Total** | ' + detectedVulnerabilities.length + ' |
...
';
  ...
  const critical = detectedVulnerabilities.filter((v) => v.severity === 'critical');
...
';
  const high = detectedVulnerabilities.filter((v) => v.severity === 'high');
