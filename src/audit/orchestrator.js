...
import { showWorkspace, WorkspaceOptions } from '../cli/workspace.js';
...
export async function startAudit(options: AuditOptions): Promise<AuditResult> {
  ...
  if (options.simulateExploits) {
    ...
    console.log(chalk.yellow('\n[3/4] Skipping exploit simulation (use --simulate-exploits to enable)\n'));
  }
  ...
  const reportPath = await writeReport(
    ...
    staticResults,
    {
      ...
      toolExecutionTime: {
        slither:
          staticResults.find((r) => r.tool === 'Slither')?.executionTime || 0,
      },
  ...
  await saveSession(workspacePath, contractContext, allFindings, reportPath);
...
async function runStaticAnalysis(
  ...
  try {
    ...
    slitherSpinner.succeed(
      `Slither complete (${((Date.now() - SlitherStart) / 1000).toFixed(1)}s)`
    );
  } catch (error: any) {
